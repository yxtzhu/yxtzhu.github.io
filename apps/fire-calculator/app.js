...
const TABS = ["Inputs", "Spending", "Projection", "Healthcare", "Scenarios", "Cities", "⚙"];
function CitiesTab({
  savings,
  swr,
  fatAnnualHere,
...
Return ONLY the JSON array, no other text.`;
    try {
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-05-20:generateContent?key=${geminiKey}`, {
        method: "POST",
        headers: {
...
function App() {
  ...
  const hcProjection = useMemo(() => {
    const d = [];
    for (let a = age; a <= 90; a++) {
      const projected = hcAtAge(a, projectedFireAge, healthMonthly, null);
      const flatInfl = currentHealthAnnual * Math.pow(1 + inf / 100, a - age);
      const phase = a < projectedFireAge ? "working" : a < 65 ? "aca" : a < 75 ? "medicare" : "ltc";
      const phaseLabel = a < projectedFireAge ? "Working" : a < 65 ? "ACA" : a < 75 ? "Medicare" : "LTC";
      d.push({
        age: a,
        projected: Math.round(projected),
        flatInfl: Math.round(flatInfl),
        phase,
        phaseLabel,
        extra: Math.round(Math.max(projected - flatInfl, 0))
      });
    }
    return d;
  }, [age, projectedFireAge, healthMonthly, currentHealthAnnual, inf]);
  ...
  const {
    ...
    if (savings >= fireTargets.regular) return {
      coastMilestone: savings,
      coastHorizonAge: age
    };
    let nc = savings;
    for (let y = 1; y <= 60; y++) {
      nc = nc * (1 + nomRet);
      if (nc >= fireTargets.regular) {
        return {
          coastMilestone: fireTargets.regular / Math.pow(1 + nomRet, y),
          coastHorizonAge: age + y
        };
      }
    }
    return {
      coastMilestone: fireTargets.regular,
      coastHorizonAge: age + 60
    };
  }, [fireTargets.regular, savings, nomRet, age]);
  const coastYrs = useMemo(() => {
    let pv = savings;
    for (let y = 0; y <= 60; y++) {
      if (pv >= coastMilestone) return y;
      pv = pv * (1 + realRet) + contrib;
    }
    return 61;
  }, [savings, coastMilestone, contrib, realRet]);
  ...
  const scenarios = useMemo(() => [{
    ...
    sr: Math.min(sr + 15, 70),
    ret,
    inf,
    color: "#9B8EC4"
  }].map(s => {
    const rr = (s.ret - s.inf) / 100;
    const c = income * (s.sr / 100) * (1 - tax / 100);
    let pv = savings;
    for (let y = 0; y <= 60; y++) {
      if (pv >= fireTargets.regular) return {
        ...s,
        years: y,
        fireAge: age + y
      };
  ...
  }), [sr, ret, inf, income, tax, fireTargets.regular, savings, age]);
  const coastData = useMemo(() => {
    const d = [];
    let wc = savings,
      nc = savings;
    const n = Math.max(coastHorizonAge - age + 5, 20);
    for (let y = 0; y <= n; y++) {
      d.push({
        age: age + y,
        withContrib: Math.round(wc),
        noContrib: Math.round(nc),
        target: Math.round(fireTargets.regular)
      });
      wc = wc * (1 + nomRet) + contrib;
      nc = nc * (1 + nomRet);
    }
    return d;
  }, [savings, contrib, nomRet, fireTargets.regular, age, coastHorizonAge]);
  ...
  const HcTip = ({
    ...
    if (!active || !payload?.length) return null;
    const d = hcProjection.find(r => r.age === label);
    return React.createElement("div", {
      style: {
        background: "#161714",
        border: "1px solid #2C2D29",
    ...
    }, "Age ", label, " \u2014 ", d?.phaseLabel), payload.map(p => React.createElement("div", {
      key: p.dataKey,
      style: {
        color: p.color
      }
    }, p.name, ": ", fmt(p.value))), d?.extra > 0 && React.createElement("div", {
      style: {
        color: "#E07B6A",
        marginTop: "0.2rem"
      }
    }, "+", fmt(d.extra), " above flat inflation"));
  };
  const phaseColors = {
    working: "#2C2D29",
    aca: "rgba(201,169,110,0.07)",
    medicare: "rgba(126,184,164,0.07)",
    ltc: "rgba(224,123,106,0.12)"
  };
  return React.createElement("div", {
    style: {
      "--bg": "#0E0F0D",
  ...
  }))), tab === "Spending" && React.createElement("div", {
    className: "fu",
    style: {
      display: "flex",
      flexDirection: "column",
  ...
  }, fmt(totalAnnual / 12), "/mo avg \xB7 feeds all FIRE targets \xB7 healthcare projected separately")), React.createElement("div", {
    className: "card"
  }, React.createElement("div", {
    className: "sl"
  ...
  }, fmt(totalAnnual)), [["lean", fireTargetsBase.lean], ["regular", fireTargetsBase.regular], ["fat", fireTargetsBase.fat]].map(([k, t]) => React.createElement("td", {
    key: k,
    style: {
      textAlign: "right",
      fontFamily: "var(--serif)",
  ...
  }, fmt(fireTargets[k]))))))))), tab === "Projection" && React.createElement("div", {
    className: "fu",
    style: {
      display: "flex",
      flexDirection: "column",
  ...
  }))))), tab === "Healthcare" && React.createElement("div", {
    className: "fu",
    style: {
      display: "flex",
      flexDirection: "column",
    ...
    className: "sl"
  }, "Healthcare Cost Phases"), React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: "0.5rem"
    }
  }, HC_PHASES.map((p, i) => React.createElement("div", {
    key: p.key,
    style: {
      background: "var(--surface2)",
      borderRadius: 5,
  ...
  }, "~", fmt(p.baseMonthly), "/mo base")), React.createElement("div", {
    style: {
      fontSize: "0.65rem",
      color: "var(--muted)",
      lineHeight: 1.4
  ...
  }, (p.annualGrowth * 100).toFixed(1), "%/yr healthcare inflation within phase"))))), React.createElement("div", {
    className: "card",
    style: {
    ...
    className: "sl"
  }, "Impact on Your FIRE Plan"), React.createElement("div", {
    className: "g2",
    style: {
      marginBottom: "0.75rem"
    }
  }, [{
    label: "Today's HC Spend",
    value: fmt(currentHealthAnnual),
    sub: `${fmt(healthMonthly)}/mo · your input`
  }, {
    label: "HC at ACA Phase",
    value: fmt(hcAtAge(projectedFireAge, projectedFireAge, healthMonthly, null)),
    sub: `At FIRE age ${projectedFireAge}, pre-Medicare`
  }, {
    label: "HC at Medicare",
    value: fmt(hcAtAge(65, projectedFireAge, healthMonthly, null)),
    sub: "Age 65, Part B + Medigap"
  }, {
    label: "HC at LTC Phase",
    value: fmt(hcAtAge(75, projectedFireAge, healthMonthly, null)),
    sub: "Age 75, Medicare + LTC risk"
  }].map(s => React.createElement("div", {
    key: s.label,
    className: "card",
    style: {
  ...
  }, "Total HC Adjustment to FIRE Target"), React.createElement("span", {
    style: {
      fontFamily: "var(--serif)",
      fontSize: "1.1rem",
      color: "#E07B6A"
  ...
  }, "NPV of projected healthcare costs above flat inflation, capitalised over 30-yr retirement at ", ret, "% nominal return. Added to all FIRE targets."))), React.createElement("div", {
    className: "card"
  }, React.createElement("div", {
    className: "sl"
  }, "Annual Healthcare Cost: Age ", age, " \u2192 90"), React.createElement("div", {
    style: {
      display: "flex",
      gap: "0.75rem",
      marginBottom: "0.75rem",
  ...
  }), l.label))), React.createElement(ResponsiveContainer, {
    width: "100%",
    height: 260
    ...
    content: React.createElement(HcTip, null)
  }), projectedFireAge > age && projectedFireAge < 90 && React.createElement(ReferenceLine, {
    x: projectedFireAge,
    stroke: "var(--muted2)",
    strokeDasharray: "3 3",
    label: {
      value: `FIRE ${projectedFireAge}`,
      fill: "#7A7B74",
      fontSize: 8,
      position: "top"
    }
  ...
  }, "Shaded area above dashed line = excess healthcare cost above general inflation. This excess is what's capitalised into your FIRE target adjustment."))), tab === "Scenarios" && React.createElement("div", {
    className: "fu",
    style: {
      display: "flex",
      flexDirection: "column",
  ...
  }, "Paths to Regular FIRE: ", React.createElement("span", {
    style: {
      color: "var(--gold)",
      fontFamily: "var(--serif)"
    }
  }, fmt(fireTargets.regular)), " (HC-adjusted)"), scenarios.map(s => React.createElement("div", {
    key: s.label,
    className: "card"
  }, React.createElement("div", {
    style: {
  ...
  }, s.years >= 60 ? "60+ yrs" : `Age ${s.fireAge}`)), React.createElement("div", {
    className: "g3",
    style: {
      marginBottom: "0.55rem"
    }
  }, [["Return", `${s.ret}%`], ["Inflation", `${s.inf}%`], ["Save Rate", `${s.sr}%`]].map(([l, v]) => React.createElement("div", {
    key: l,
    style: {
      background: "var(--surface2)",
      borderRadius: 4,
  ...
  }, s.years >= 60 ? "60+ years away" : `${s.years} yrs away`), React.createElement("div", {
    className: "bar"
  }, React.createElement("div", {
    className: "bar-fill",
    style: {
      width: `${pct(savings, fireTargets.regular)}%`,
      background: s.color
    }
  })))), React.createElement("div", {
    className: "card"
    ...
    className: "sl"
  }, "Savings Rate Sensitivity"), React.createElement("div", {
    className: "g3",
    style: {
      gap: "0.4rem"
    }
  }, [20, 25, 30, 35, 40, 50].map(s => {
    const c = income * (s / 100) * (1 - tax / 100);
    let pv = savings,
      yrs = 61;
    for (let y = 0; y <= 60; y++) {
      if (pv >= fireTargets.regular) {
        yrs = y;
        break;
      }
      pv = pv * (1 + realRet) + c;
    ...
    return React.createElement("div", {
      ...
      style: {
        fontFamily: "var(--serif)",
        fontSize: "1rem",
        color: cur ? "var(--gold)" : "var(--text)"
      }
    }, yrs >= 60 ? "60+" : yrs, React.createElement("span", {
      style: {
        fontSize: "0.58rem",
        color: "var(--muted2)",
        marginLeft: 2
      }
    }, "yrs")), React.createElement("div", {
      style: {
        fontSize: "0.57rem",
        color: "var(--muted2)"
      }
    }, "age ", yrs >= 60 ? "60+" : age + yrs));
  })))), tab === "Cities" && React.createElement(CitiesTab, {
    savings: savings,
    swr: swr,
